...
(function () {
  ...
  function positionFooterLogo(footerLogo, footer, footerContainer) {
    ...
    var isLargeScreen = window.innerWidth > TABLET_MAX_WIDTH;
    if (!isLargeScreen) {
    ...
    var containerRect = footerContainer.getBoundingClientRect();
    footerLogo.style.display = '';
    footerLogo.style.height = FOOTER_LOGO_HEIGHT + 'px';
    footerLogo.style.left = containerRect.left + 'px';
    footerLogo.style.top = footerRect.top + (footerRect.height - FOOTER_LOGO_HEIGHT) / 2 + 'px';
  }
  ...
  function createPositionAll(pieces, anchors) {
    return function positionAll() {
      ...
      positionFooterLogo(pieces.footerLogo, anchors.footer, anchors.footerContainer);
    };
  }
